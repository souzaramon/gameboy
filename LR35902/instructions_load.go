...
func LD_r8_n8(cpu *CPU, r8 string) {
	cpu.R.SetByName8(r8, cpu.Memory.Read8(cpu.R.PC))
	cpu.R.PC += 1
	cpu.Cycles += 4
}
...
func LD_r16_n16(cpu *CPU, r16 string) {
	cpu.R.SetByName16(r16, cpu.Memory.Read16(cpu.R.PC))
	cpu.R.PC += 2
	cpu.Cycles += 8
}
...
func LD_HLI_A(cpu *CPU) {
	...
	cpu.Memory.Write8(HL, cpu.R.GetByName8(R_A))
...
func LD_A_HLI(cpu *CPU) {
	...
	cpu.R.SetByName8(R_A, cpu.Memory.Read8(HL))
	cpu.R.SetByName16(R_HL, HL+1)
}
...
func LD_n16_SP(cpu *CPU) {
	addr := cpu.Memory.Read16(cpu.R.PC)
	cpu.Memory.Write16(addr, cpu.R.GetByName16(R_SP))
	cpu.R.PC += 2
}